...
if ! _check_and_install_utils; then
...
current_timestamp=$(date -d "$starting_date" "+%Y-%m-%d_%H-%M-%S")

# Set backup info variables
backup_info="$output_path/backup-info.log"
backup_file="$output_path/$current_timestamp.tar.zst"
...
if ! _compress "$input_path" "$backup_file" "${excluded_array[@]}"; then
  _log_status_to_file "Backup failed, failed to compress files" "$backup_info" "$input_path" "$output_path" "$backup_file" "$starting_date" "$backups_to_keep"
  exit 1
fi
...
for backup in "${backups[@]}"; do
  ...
  if ! rm -f "$output_path/$backup"; then
    _log_status_to_file "Backup failed, failed to remove old backups" "$backup_info" "$input_path" "$output_path" "$backup_file" "$starting_date" "$backups_to_keep"
    exit 1
  fi
done
...
_log_status_to_file "Backup created successfully" "$backup_info" "$input_path" "$output_path" "$backup_file" "$starting_date" "$backups_to_keep"
